...
def concatena_arquivos():
    ...
    for csvfile in glob.glob("/mnt/c/airflow/dags/move_files/resolucoes/*.csv"):
        extracted_data = extracted_data.append(extract_from_csv(csvfile), ignore_index=True)
        #df_empresas = df_empresas.append(df, ignore_index=True)

    correto = extracted_data[['RESOLUCAO', 'EMPRESA', 'AUTORIZACAO', 'NOME_PRODUTO_MARCA', 'NUMERO_PROCESSO', 'NUMERO_REGISTRO', 'VENDA_EMPREGO',
                                     'VENCIMENTO', 'APRESENTACAO', 'VALIDADE_PRODUTO', 'CATEGORIA', 'ASSUNTO_PETICAO', 'EXPEDIENTE_PETICAO', 'VERSAO']]

    correto['VENCIMENTO'] = pd.to_datetime(correto['VENCIMENTO'], format="%m/%Y")
    
    correto.to_excel(f"/mnt/c/airflow/dags/move_files/cliente/Resolucao_mes_{month - 1}{date_for_file1}.xlsx", sheet_name="Sheet1")
...
def envia_email(email_from, email_to, password):
    ...
    try:
        ...
        msg['From'] = email
        msg['To'] = cliente
        msg['Subject'] = "Arquivo.xlsx das resolucoes referentes aos meses de Janeiro e Fevereiro"
        
        body = "\nSegue em Anexo o arquivo referente as resolucoes de Janeiro e Fevereiro."

        msg.attach(MIMEText(body, 'plain'))

        source = '/mnt/c/airflow/dags/move_files/cliente'
...
def log(message):
    ...
    with open('/mnt/c/airflow/dags/move_files/logfile.txt', 'a') as f:
