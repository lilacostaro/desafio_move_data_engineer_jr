...
def etl_secundario():
    ...
    exporta_dataframe(df_categorias, 'database_backup\Categorias')
    ...
    exporta_dataframe(df_assuntos, 'database_backup\Assuntos')
    log('Dataframe gerado e exportado com sucesso!')
    print(df_assuntos)
    ...
    exporta_dataframe(df_saneantes, 'database_backup\Saneantes')
    log('Dataframe gerado e exportado com sucesso!')
    print(df_saneantes)
...
etl_principal()
etl_secundario()
envia_email_e_move_arquivos()
finish = datetime.now()
log(f'Processo realizado com sucesso! Tempo total de execucao: {finish - start}\n')